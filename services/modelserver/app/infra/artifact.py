...
class ArtifactStore:
    def __init__(
        ...
        *,
        ...
        self.endpoint = (endpoint or os.getenv("MINIO_ENDPOINT", "http://minio:9000")).removeprefix("http://").removeprefix("https://")
        self.secure = os.getenv("MINIO_ENDPOINT", "http://minio:9000").startswith("https://")
        self.bucket = bucket
    ...
    def verify_and_load(self) -> ModelArtifact:
        ...
        expected = str(model_card.get("sha256", ""))
        computed = hashlib.sha256(weights).hexdigest()
        if expected.removeprefix("sha256:") != computed:
            raise BootValidationError("Model artifact SHA-256 does not match the model card.")

        return ModelArtifact(
            model_name=str(model_card.get("model_name", "unknown")),
            version=str(model_card.get("version", "unknown")),
            sha256=computed,
            weights=weights,
...
def assert_nonzero_thresholds(path: Path | None = None) -> None:
    threshold_path = path or Path(__file__).resolve().parents[4] / "eval_thresholds.yaml"
